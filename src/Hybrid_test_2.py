...
# For drawing hand skeleton in gesture mode
...
# Gesture mode state
gesture_mode = False
last_chord_count = -1  # Track last played chord to avoid replaying
chord_display = ""
finger_count_text = ""
...
def count_raised_fingers(hand_landmarks, hand_label, frame_shape):
    ...
    # Other 4 fingers: tip.y < pip.y = raised
    finger_tips = [
...
while True:
    ...
    if gesture_mode:
        ...
        # ====== TOUCH MODE: desk press detection ======
        # Desk edge detection
    ...
    if key == ord('q'):
        ...
        last_chord_count = -1  # Reset chord tracking on switch
        # Re-init touch states when switching
        touch_states_left = [TouchStateMachine() for _ in range(5)]
        touch_states_right = [TouchStateMachine() for _ in range(5)]
        if not gesture_mode:
